export default class Time{
     ...
     * @param {boolean} thisDay 
     * Если true возвращает строковое представление указанного дня недели, по умолчанию - false
     * @param {number} day 
     * Номер месяца от 1 до 7
     * @param {string} locale 
     * Параметр локализации
     * ---
     * @returns {string|Array<string>}
     * Возвращает строку (при thisDay=true), либо массив строк
     */
    static getDayInWeekString(thisDay=false,day=1,locale='ru'){
        if(day<1||day>7)
        throw new Error('Day out of range exception')
        if(Time.#weekDaysString.has(locale)){
            if(thisDay){
                let monthes = Time.#weekDaysString.get(locale)
                return monthes[day-1]
            }else{
    ...
    */
    ...
    #reverseISO(str){
        ...
        if(!isCheck)
            throw new Error('Не удалось создать объект времени, неверный формат строки')
        this.#calcTicks({
            year:objDate.year,
            month:objDate.month,
            day:objDate.day,
